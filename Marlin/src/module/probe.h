...
public:
    ...
    static float run_z_probe_multipleZHomeing(const bool sanity_check, const float z_min_point, const float z_clearance, uint8_t probing_iterations) { return run_z_probe(sanity_check, z_min_point, z_clearance, probing_iterations); }
...
private:
    ...
    static float run_z_probe(const bool sanity_check=true, const float z_min_point=Z_PROBE_LOW_POINT, const float z_clearance=Z_TWEEN_SAFE_CLEARANCE, uint8_t probing_iterations = 0);
  #endif
};
